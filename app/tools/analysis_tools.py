...
def generate_biomass_image(run_context: RunContext, car_codes: list[str]) -> ToolResult:
    ...
    try:
        ...
        log_debug(f"generate_biomass_image: mapa gpw gerado ({selected_property.car_code})")
        return ToolResult(
            content=("Legenda: Acumulado de biomassa no ano de referência. Azul claro (Alta concentração) a Roxo escuro (Baixa concentração)."),
            images=[Image(content=buffer.getvalue())]
        )
...
@tool(tool_hooks=[validate_selected_property_hook], description=get_tool_description("analysis_tools", "generate_pasture_classification_image"))
...
def get_pasture_biomass_history(run_context: RunContext, car_codes: list[str]) -> ToolResult:
    ...
    """
    log_debug(f"get_pasture_biomass_history: car_codes={car_codes}")
    try:
        all_properties = run_context.session_state['all_properties']
        selected_property = next((prop for prop in all_properties if prop["car_code"] == ', '.join(car_codes)), None)
        if selected_property is None:
        ...
        log_debug(f"get_pasture_biomass_history: histórico gerado ({selected_property.car_code})")
