...
# Assumindo a importação correta do ToolResult do seu framework
# from agno.utils import ToolResult 
...
def update_user_information(key: str, title: str, info: str, run_context: RunContext):
    """
    Atualiza ou insere uma característica/preferência na persona do usuário dentro da sessão.
    """
    session_state = run_context.session_state
    user_persona = session_state.get("user_persona", {})
    ...
    return ToolResult(content=f"Sucesso: A característica '{title}' (Chave: '{key}') foi atualizada no perfil do usuário.")
