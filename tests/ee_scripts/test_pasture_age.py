"""
Teste de integração real (GEE + Xee) para a idade de pastagem on-the-fly.
...
(inicializa o Earth Engine via `.env` real na raiz do projeto).
    ...
    .venv/bin/python -m pytest tests/ee_scripts/test_pasture_age.py -v
"""
import json
import shutil
...
def test_estimate_pasture_age_on_the_fly_produces_plausible_values(index):
    prop = _load_test_properties()[index]

    result = estimate_pasture_age_on_the_fly(roi=prop["roi"], car_code=prop["car_code"])
    ...
    area_by_class = result["area_by_age_class_ha"]
    assert area_by_class, "Nenhuma área de pastagem mapeada — resultado suspeito"
    assert set(area_by_class.keys()) <= set(_AGE_DICT.values())
...
def test_estimate_pasture_age_on_the_fly_uses_cache_on_second_call():
    ...
    first = estimate_pasture_age_on_the_fly(roi=prop["roi"], car_code=prop["car_code"])
    assert first["cached"] is False
    assert Path(zarr_path).exists() and Path(png_path).exists()

    second = estimate_pasture_age_on_the_fly(roi=prop["roi"], car_code=prop["car_code"])
    assert second["cached"] is True
    assert second["area_by_age_class_ha"] == first["area_by_age_class_ha"]
