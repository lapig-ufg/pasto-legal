...
from app.utils.scripts.boletim_scripts import build_boletim_chat_summary, build_boletim_story
...
def _safe_fetch_image(label: str, fetch_fn):
    ...
    try:
        ...
        print(f"WARNING: {label} indisponível para o boletim: {error}", flush=True)
        return None
...
def generate_property_boletim(run_context: RunContext, car_codes: list[str]) -> ToolResult:
    ...
    try:
        ...
        pasture_stats = query_pasture_statistics(coords=coords, year=today.year, month=today.month)
        ...
        def _fetch_biomass_image():
            return retrieve_t2g_biomass_image(coords, today.month, today.year) or retrieve_mapbiomas_biomass_image(coords, year=2024)
