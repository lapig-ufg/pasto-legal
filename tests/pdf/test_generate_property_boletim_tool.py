...
def _build_context_with_property(rural_property: Feature) -> StateContext:
    return StateContext(
        session_state={
...
def test_generate_property_boletim_returns_valid_pdf_file():
    ...
    result = generate_property_boletim(run_context=run_context, feature_id=rural_property.id)
...
def test_generate_property_boletim_reports_friendly_error_without_property():
    run_context = StateContext(session_state={})

    result = generate_property_boletim(run_context=run_context, feature_id="Nenhuma Propriedade")
