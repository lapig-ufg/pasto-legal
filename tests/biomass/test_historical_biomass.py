...
def test_synthetic_pixel_converts_gpp_to_dry_matter():
    """
    1000 gC/m²/ano x escala 1,0 x CUE 0,45 x 2,7 x 0,01 = 12,15 t MS/ha/ano.
    """
    assert gpp_to_dry_matter_t_ha(1000.0) == pytest.approx(12.15, abs=1e-9)
...
def test_carbon_use_efficiency_is_mandatory_in_the_chain():
    """
    Sem a eficiência do uso do carbono a conversão superestima em >2x.

    O produto é GPP bruto: aplicar só carbono -> matéria seca trata toda a fixação
    de carbono como forragem colhível.
    """
    with_cue = gpp_to_dry_matter_t_ha(1830.97)
    without_cue = gpp_to_dry_matter_t_ha(1830.97, carbon_use_efficiency=1.0)

    assert without_cue / with_cue == pytest.approx(1 / CARBON_USE_EFFICIENCY)
    assert without_cue > 45.0
    # Com a CUE o valor fica próximo do MapBiomas medido no imóvel (21,08).
    assert with_cue == pytest.approx(22.25, abs=0.1)
...
def test_conversion_factors_are_all_named():
    ...
    assert set(factors) == {
        "asset_scale", "carbon_use_efficiency", "carbon_to_dry_matter", "unit_conversion",
    }
    assert factors["asset_scale"] == 1.0, "o produto histórico não tem escala a aplicar"
    assert factors["carbon_use_efficiency"] == CARBON_USE_EFFICIENCY


def test_uncertainty_bounds_come_from_the_cue_range():
    lower, upper = historical_uncertainty_bounds(22.25)

    assert lower == pytest.approx(22.25 * CARBON_USE_EFFICIENCY_MIN / CARBON_USE_EFFICIENCY)
    assert upper == pytest.approx(22.25 * CARBON_USE_EFFICIENCY_MAX / CARBON_USE_EFFICIENCY)
    assert lower < 22.25 < upper


def test_uncertainty_range_brackets_the_mapbiomas_reference():
    """A faixa de CUE tem que conter o valor do MapBiomas no imóvel de referência."""
    lower, upper = historical_uncertainty_bounds(gpp_to_dry_matter_t_ha(1886.2))

    assert lower <= 21.08 <= upper
...
def test_historical_contract_declares_annual_support_and_thirty_metres():
    ...
    assert "gC/m2" in GPW_UGPP_HISTORICAL_CONTRACT.native_unit
...
def _key(**overrides) -> str:
    kwargs = dict(
        ...
        mask=_FakeMask(), carbon_use_efficiency=CARBON_USE_EFFICIENCY, scale=30.0,
    )
    kwargs.update(overrides)
...
def test_export_key_changes_with_every_input_that_changes_the_result():
    ...
    assert _key() != _key(carbon_use_efficiency=0.40)
...
def test_historical_series_matches_the_known_property(covered_roi_fixture=None):
    ...
    for estimate in series:
        ...
        # MapBiomas registra 20-22 t MS/ha/ano neste imóvel; a estimativa fica
        # próxima disso, com o viés de +12% documentado no contrato.
        assert 15.0 < estimate.value_per_ha < 30.0
...
def test_pixel_export_round_trips_through_zarr(tmp_path, monkeypatch):
    ...
    for key in (
        ...
        "pasture_mask_source", "model_version", "factor_carbon_use_efficiency",
    ):
